// VIEW — แสดงการ์ด + 3D flip, รับ callbacks จาก props
import { useState, useEffect } from "react";
import { LEVEL_BADGE, CATEGORIES } from "../../constants";

export default function FlashCard({ card, onKnow, onAgain }) {
  ...
  const handleMark = (knew) => {
    if (!flipped) return;
    setLeaving(true);
    setTimeout(() => { setLeaving(false); knew ? onKnow() : onAgain(); }, 280);
  };
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full">
      ...
      <div className={`flex gap-3 w-full max-w-lg transition-all duration-300
      ...
      {!flipped && (
        <p className="font-mono text-[10px] text-white/15 tracking-widest uppercase">tap card to flip</p>
      )}
    </div>
  );
}
