...
export default function DeckEditor() {
  const { deck, addCard, editCard, removeCard, error, setError, searchCards } = useDeck();
  const { user } = useAuth();
  ...
  // Auto-fetch from Dictionary API
  const handleFetch = async () => {
    const result = await fetchWord(form.word);
    if (result) setForm((f) => ({ ...f, word:result.word, phonetic:result.phonetic, meaning:result.meaning, example:result.example }));
  };
  ...
  const saveEdit = async () => {
    await editCard(editId, editData);
    setEditId(null);
  };

  const filtered = searchCards(search);
  ...
  return (
    <div className="w-full max-w-2xl">
      ...
      <button onClick={() => setOpen((o) => !o)}
        ...
        {open ? "▲" : "▼"} Manage Deck ({deck.length} cards)
      </button>
        ...
        <div className="mt-4 space-y-4 animate-fadeUp">
          ...
          {tab === "add" && (
            <div className="p-5 rounded-2xl border border-white/8 bg-white/3 space-y-3">
              {!user && (
                <p className="font-mono text-xs text-amber-400/70 bg-amber-400/8 border border-amber-400/20 rounded-xl px-3 py-2">
                  ⚠️ เข้าสู่ระบบก่อนเพื่อเพิ่มการ์ดส่วนตัว
                </p>
              )}
              {/* Word + Auto-fetch */}
              <div className="flex gap-2">
                <input value={form.word} onChange={(e) => setField("word", e.target.value)}
              ...
              <input value={form.phonetic} onChange={(e) => setField("phonetic", e.target.value)}
                placeholder="/fəˈnetɪk/ (optional)" className={inputCls} />
              <input value={form.meaning} onChange={(e) => setField("meaning", e.target.value)}
                placeholder="Meaning / คำแปล…" className={inputCls} />
              <textarea value={form.example} onChange={(e) => setField("example", e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.level} onChange={(e) => setField("level", e.target.value)} className={selectCls}>
              ...
              <button onClick={handleAdd} disabled={!form.word.trim() || !form.meaning.trim() || !user}
                className="w-full py-2.5 rounded-xl font-mono text-xs uppercase tracking-widest
                           ...
                           hover:bg-white/15 disabled:opacity-25 disabled:cursor-not-allowed
                    ...
                    <div key={card.id} className="p-4 rounded-xl border border-blue-400/25 bg-blue-400/4 space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <input value={editData.word} onChange={(e) => setEditData({ ...editData, word: e.target.value })}
                          placeholder="Word" className="bg-white/5 border border-white/8 rounded-lg px-3 py-2 font-mono text-sm text-white focus:outline-none" />
                        <input value={editData.phonetic} onChange={(e) => setEditData({ ...editData, phonetic: e.target.value })}
                          placeholder="Phonetic" className="bg-white/5 border border-white/8 rounded-lg px-3 py-2 font-mono text-sm text-white/60 focus:outline-none" />
                      </div>
                      <input value={editData.meaning} onChange={(e) => setEditData({ ...editData, meaning: e.target.value })}
                        placeholder="Meaning" className="w-full bg-white/5 border border-white/8 rounded-lg px-3 py-2 font-mono text-sm text-white focus:outline-none" />
                      <textarea value={editData.example} onChange={(e) => setEditData({ ...editData, example: e.target.value })}
                        placeholder="Example" rows={2}
                        className="w-full bg-white/5 border border-white/8 rounded-lg px-3 py-2 font-mono text-sm text-white/60 resize-none focus:outline-none" />
                      <div className="flex gap-2">
                        <select value={editData.category} onChange={(e) => setEditData({ ...editData, category: e.target.value })}
                          className="flex-1 bg-[#0d1117] border border-white/8 rounded-lg px-3 py-2 font-mono text-xs text-white/60 focus:outline-none">
                          {CATEGORIES.filter((c) => c.value !== "all").map((c) => (
                            <option key={c.value} value={c.value}>{c.emoji} {c.label}</option>
                          ))}
                        </select>
                      ...
                      <div className="flex gap-2">
                        <button onClick={saveEdit}
                          className="flex-1 py-2 rounded-lg font-mono text-xs uppercase tracking-widest
                                     ...
                                     hover:bg-emerald-500/25 active:scale-95 transition-all">Save</button>
                        <button onClick={() => setEditId(null)}
                          className="flex-1 py-2 rounded-lg font-mono text-xs uppercase tracking-widest
                                     border border-white/8 text-white/30 hover:bg-white/5 active:scale-95 transition-all">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    /* View mode */
                    <div key={card.id}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl border border-white/4
                      ...
                      <div className="flex-1 min-w-0 space-y-0.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`font-mono text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-wide ${LEVEL_BADGE[card.level] || "bg-white/5 text-white/25 border-white/10"}`}>
                            {card.level}
                          </span>
                          {card._source === "shared" && (
                            <span className="font-mono text-[9px] px-1.5 py-0.5 rounded border border-purple-400/20 text-purple-400/50">built-in</span>
                          )}
                        </div>
                      ...
                      {/* Only show edit/delete for personal cards */}
                      {card._source === "personal" && (
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => startEdit(card)}
                            className="px-2.5 py-1.5 rounded-lg font-mono text-[10px] text-blue-400/60 hover:text-blue-400 hover:bg-blue-400/8 transition-all">
                            edit
                          </button>
                          <button onClick={() => removeCard(card.id)}
                            className="px-2.5 py-1.5 rounded-lg font-mono text-[10px] text-red-400/60 hover:text-red-400 hover:bg-red-400/8 transition-all">
                            ✕
                          </button>
                        </div>
                      )}
