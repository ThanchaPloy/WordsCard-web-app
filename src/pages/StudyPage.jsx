...
export default function StudyPage() {
  ...
  const filtered = deck.filter((c) => {
    ...
    const levelOk = filterLevel === "all" || c.level    === filterLevel;
  ...
  const startSession = (cards) => {
    ...
    setMissed([]); setCurrent(0); setCorrect(0); setTotal(0);
    setPhase("studying");
  };

  const handleKnow  = () => { setCorrect((c) => c + 1); setTotal((t) => t + 1); advance(); };
  const handleAgain = () => { setMissed((m) => [...m, queue[current]]); setTotal((t) => t + 1); advance(); };
  const advance = () =>
    setCurrent((c) => { if (c + 1 >= queue.length) { setPhase("done"); return c; } return c + 1; });

  const restartAll   = () => startSession(filtered);
  const restartWrong = () => { if (missed.length === 0) return restartAll(); startSession(missed); };
  const goHome       = () => setPhase("home");

  const catStats = CATEGORIES.filter((c) => c.value !== "all").map((c) => ({
    ...c,
    count: deck.filter((d) => d.category === c.value).length,
  }));
    ...
    <div
      ...
      style={{
        background:
          ...
          "radial-gradient(ellipse at 85% 20%,rgba(139,92,246,0.05) 0%,transparent 55%)," +
          "#07090f",
      }}
    >
      {/* ── Header ── */}
      <header className="w-full max-w-2xl flex items-center justify-between px-4 sm:px-6 pt-6 pb-4">
        <div>
          ...
          <p className="font-mono text-[10px] text-white/25 tracking-[0.2em] uppercase mt-0.5">
            English Vocabulary
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          {phase === "studying" && (
            <button
              onClick={goHome}
              className="font-mono text-xs text-white/25 hover:text-white/50 transition-colors uppercase tracking-widest"
            >
              ✕ Exit
            </button>
          )}

          {/* Auth: show UserMenu if logged in, else login button */}
          {user ? (
            <UserMenu />
          ) : (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-xl
                         font-mono text-xs uppercase tracking-widest text-white/50
      ...
      <main className="w-full max-w-2xl flex-1 flex flex-col items-center gap-6 px-4 sm:px-6 pb-12">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center">
            <p className="font-mono text-sm text-white/25 animate-pulse tracking-widest uppercase">
              Loading vocabulary…
            </p>
          </div>
          ...
          <DoneScreen
            correct={correct} total={total} missed={missed}
            onRestartAll={restartAll} onRestartWrong={restartWrong} onGoHome={goHome}
          />

        ) : phase === "studying" ? (
          <>
            <ProgressBar current={current} total={queue.length} correct={correct} />
            <FlashCard card={queue[current]} onKnow={handleKnow} onAgain={handleAgain} />
          ...
          <div className="w-full space-y-6 animate-fadeUp">
            ...
            {/* Login banner (only if not logged in) */}
            {!user && (
              <div
                className="w-full p-4 rounded-2xl flex items-center justify-between gap-4"
                style={{ background: "rgba(59,130,246,0.07)", border: "1px solid rgba(59,130,246,0.15)" }}
              >
                <div>
                  <p className="font-mono text-xs text-blue-300/80">เข้าสู่ระบบเพื่อบันทึกการ์ดส่วนตัว</p>
                  <p className="font-mono text-[10px] text-white/25 mt-0.5">คุณสามารถท่องคำศัพท์ได้โดยไม่ต้อง login</p>
                </div>
                <button
                  onClick={loginWithGoogle}
            ...
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/25 mb-3">
                Study by Category
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                >
                  ...
                  <span className="font-mono text-xs text-blue-400/60 group-hover:text-blue-400 transition-colors">
                    ▶ Start
                  </span>
                </button>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: "Categories",    value: [...new Set(deck.map((c) => c.category))].length },
